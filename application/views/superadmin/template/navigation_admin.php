            ...
            <div class="navbar-container content">
                <div class="navbar-collapse" id="navbar-mobile">
                    ...
                    <ul class="nav navbar-nav float-right">
                        ...
                        <?php 
                            $this->db->select('tb_pengaduan.id_pengaduan, tb_pengaduan.subjek, tb_pengaduan.tanggal_pengaduan, login_session.id_user, login_session.nama');
                            $this->db->from('tb_pengaduan');
                            $this->db->join('login_session', 'login_session.id_user = tb_pengaduan.id_user');
                            $this->db->where('tb_pengaduan.jawaban', null);
                            $data = $this->db->get()->result_array();
                        ?>
                        <li class="dropdown dropdown-notification nav-item"><a class="nav-link nav-link-label" href="javascript:void(0);" data-toggle="dropdown"><i class="ficon bx bx-bell bx-tada bx-flip-horizontal"></i><span class="badge badge-pill badge-danger badge-up"><?=count($data)?></span></a>
                            <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                                <li class="dropdown-menu-header">
                                    <div class="dropdown-header px-1 py-75 d-flex justify-content-between">
                                        <span class="notification-title"><?=count($data)?> Laporan Pengaduan Baru</span>
                                    </div>
                                </li>
                                            ...
                                            <div class="media d-flex align-items-center">
                                                ...
                                                <div class="media-body">
                                                    <h6 class="media-heading">
                                                        <span class="text-bold-500"><?=$pengaduan['nama']?> </span><?= $pengaduan['subjek']?>
                                                    </h6>
                                                    <small class="notification-text"><?= date('d-M-Y', strtotime($pengaduan['tanggal_pengaduan']))?></small>
                                                </div>
                                            </div>
                                        </a>
                                    <?php endforeach; ?>
                                </li>
                                <li class="dropdown-menu-footer"><a class="dropdown-item p-50 text-primary justify-content-center" ></a></li>
                            </ul>
                        </li>
