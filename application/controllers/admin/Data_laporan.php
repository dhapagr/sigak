...
class Data_laporan extends CI_Controller {
	...
	public function feedback()
	{
		...
		$id_user = $this->session->userdata('id_user');		
		$data['data_user']			= $this->Admin_model->get_dataById($id_user);
		$data['data_feedback']		= $this->db->get('tb_feedback')->result_array();
		// echo "<pre>"; print_r($data); exit;
		
		$this->load->view('admin/template/navigation_admin',$data);
		$this->load->view('admin/template/v_feedback', $data);
		$this->load->view('admin/template/footer_admin');
	}
	public function get_detail_feedback($id_feedback)
	{
		$data	= $this->db->get_where('tb_feedback', ['id_feedback' => $id_feedback])->row();
		// echo print_r($data);
		echo'<div id="modal-detail-feedback" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-lg modal-dialog-scrollable">
					<div class="modal-content">
						<div class="modal-header d-flex justify-content-between">
							<h5 class="modal-title">Detail Pengaduan</h5>
						...
						<div class="modal-body">
							<div class="row">
								<div class="col-lg-7">
									<div>
										<h6 class="text-primary"><i class="fas fa-user-circle"></i> Pengirim: <span class="text-secondary">'.$data->nama.'</span></h6>
									</div>
									<div>
										<h6 class="text-primary"><i class="fa-solid fa-at"></i></i> Email: <span class="text-secondary">'.$data->email.'</span></h6>
									</div>
									<div>
										<h6 class="text-primary"><i class="fa-solid fa-message"></i> Subject: <span class="text-secondary">'.$data->subject.'</span></h6>
									</div>
								</div>
								<div>
									...
									<div style="text-align: justify; margin-left: 20px;">
										<h6 class="text-secondary">'.$data->isi.'</h6>
									</div>
	...
	public function hapus_feedback($id_feedback)
	{
		...
		$this->db->delete('tb_feedback', $where);
		$this->Admin_model->hapus_data($where,'tb_feedback');
		...
		redirect('admin/Data_laporan/feedback');
	}
}